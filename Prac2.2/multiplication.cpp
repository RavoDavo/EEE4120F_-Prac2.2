...
//Author: David Da Costa & Theodore Psillos EEE4120F
//Prac 2.2 
...
int main(void)
{
   //Outer loop to loop through matrix sizes 50->800
	for (int i=1;i<17;i++){
   //Matrix size
	int Size = 50*i;
   

   //Create and open csv to store timings
	fstream fout;
    	fout.open("WorkGroup=Timings.csv", ios::out | ios::app);
	...
	<<(temp)
	<< "\n"
	<< "Execution time"<<","<<"Setup time"
	<< "\n";
   //inner loop to obtain 15 timings for the same parameters
	for (int j=0;j<15;j++){
   ...
   //initialize buffer
	...
	start = clock(); //start running clock for Overhead
	...
	//compile the source code obtained in step 5
	...
	size_t local_size =temp; //Size of each work group
   ...
   // initialize the output array
	...
	// create matrixA_buffer, matrixB_buffer,Size_Buffer and output_buffer, with clCreateBuffer()
   
   //Matrix A Buffer
	matrixA_buffer = clCreateBuffer(context,CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,global_size*sizeof(Size*Size), &matrixA, &err);
   //Matrix B Buffer
	matrixB_buffer = clCreateBuffer(context,CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,global_size*sizeof(Size*Size), &matrixB, &err);
   //Matrix dimension/size buffer
	size_buffer = clCreateBuffer(context,CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,sizeof(int), &Size, &err);
   //Output buffer
	output_buffer = clCreateBuffer(context,CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,global_size*sizeof(Size*Size), output, &err);
   ...
   //Setting arguments for kernel
	...
	end=clock();//end timing for overhead
	printf ("Setup took: %0.8f sec \n",((float) end - start)/CLOCKS_PER_SEC);
	double time_setup=((float) end - start)/CLOCKS_PER_SEC;
   
	start2=clock();//start execution timer
   ...
   //deploy the kernels
	...
	end2 = clock();//end execution timer
	printf ("OpenCL Execution took: %0.8f sec, matrix size = %i\n",((float) end2 - start2)/CLOCKS_PER_SEC, Size);
	double time_ex=((float) end2 - start2)/CLOCKS_PER_SEC;
